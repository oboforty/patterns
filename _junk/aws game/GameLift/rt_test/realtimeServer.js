...
const minimumElapsedTime = 80;
const maximumElapsedTime = 720; // 12 min
...
function onMessage(gameMessage) {
    switch (gameMessage.opCode) {
      case OP_CODE_CUSTOM_OP1: {
        ...
        session.getPlayers().forEach((player, playerId) => {
            let outMessage = session.newTextGameMessage(OP_CODE_GAMEOVER, session.getServerId(), "You Won!");
...
async function tickLoop() {
    ...
    if ( (activePlayers == 0) && (elapsedTime > minimumElapsedTime) || elapsedTime > maximumElapsedTime) {
